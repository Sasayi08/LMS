package org.example.app.db.memory;
...
public class MemoryDb implements DataRepository {
    ...
    private final Set<Book> allLibraryBooks = new HashSet<>(); // all the books
...
//    }
    ...
    @Override
    public Book addBook(Book book, String description, String cover_img_src) {
    ...
    @Override
    public ImmutableList<Book> getAllLibraryBooks() {
        return ImmutableList.copyOf(allLibraryBooks);
    }
    ...
    public Optional<Book> getBook(UUID id) {
        System.out.println("inside getBook (user)");
        return books.stream()
                .filter(expense -> expense.getId().equals(id))
                .findFirst();
    }
    ...
    @Override
    public Optional<Book> getBook(UUID id, Boolean isAllLibraryBooks) {
        System.out.println("inside getBook (all)");

        return allLibraryBooks.stream()
                .filter(expense -> expense.getId().equals(id))
                .findFirst();
    }
    ...
    @Override
    public void updateBook(Book updatedExpense) {
        Optional<Book> ExpenseOpt = books.stream().filter(expense -> expense.equals(updatedExpense)).findFirst();
        if (ExpenseOpt.isPresent()) {
            books.remove(ExpenseOpt.get());
            books.add(updatedExpense);
        }
    }
    ...
    public Settlement addSettlement(Settlement settlement) {
        ...
        books.add(settlement.getGeneratedExpense());
        return settlement;
    }
